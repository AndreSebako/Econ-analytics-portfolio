...
export default function FedRateChart() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!res.ok) throw new Error("Failed to load Fed rate data");
  ...
  const policySignal = useMemo(() => {
    if (latest === null) return "No Data";
    if (latest >= 5) return "Restrictive";
    if (latest >= 3) return "Tight";
    return "Easing";
  }, [latest]);

  const policyColor =
    policySignal === "Restrictive"
      ? "#f87171"
      : policySignal === "Tight"
      ? "#60a5fa"
      : "#4ade80";
    ...
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-medium" style={{ color: policyColor }}>
            {policySignal}
          </p>
        </div>
      </div>
